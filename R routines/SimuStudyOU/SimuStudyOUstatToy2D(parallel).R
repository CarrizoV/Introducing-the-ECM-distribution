...
## parallel code
##It does MGLE and MCLE at the same time 
...
for(theta in c(0.001)){
  for(N in 10^c(2:4)){
    ...
    clusterEvalQ(cl , {
      ...
      source( file.path( functions.path , "ECM_Gauss_ll.R" ) )
    ...
    L <- parLapply(cl , seeds , 
                   function(s){
                     ...
                     #X.sim <- vector( "list" , length = N.sim)
                     ...
                     for(j in 1:N.sim){
                       ...
                       #count and trajectories simulation
                       ...
                       thetas.0 <- 0.5*c( theta/10 , theta , 10*theta )
                       
                       #We retain the best obtained from such initial values
                       ll.best.mgle <- -Inf
                       ll.best.ap <- -Inf
                       #WATCH OUT YOU ARE NOT DISTINGUISHING AMONG METHODS MAGLE OR AP!!
                       ...
                       for(k in 1:length(thetas.0)){
                         ...
                         par.0 <- c(log.tau.0 , log.sigma.0 ,  z.0)
    ...
    saveRDS( Q.sim , file = paste("output/Q.sim", ", par=(log(",tau , "), log(" , sigma , "), (" , z[1] , " , " , z[2] , "))" ,
                                  ", N=" , N , ", N.sim=" , N.sim.per.core*n.cores, ".rds",  sep = "")      )
